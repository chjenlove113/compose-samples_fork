...
//    val items: Flow<PagingData<News>> =
//        showHomeChildPagingUseCase.invoke(1,navKey.slug.Key,navKey.slug.slug).flowOn(dispatcherProvider.io).cachedIn(viewModelScope)
...
//           //.collect { _uiState.value = UiState.Success(it) }
    ...
    fun fetchShowHomeChildPaging(pageNumber: Int = 1, site_Slug: String = "", cat_Slug: String = "") {
        viewModelScope.launch(dispatcherProvider.main) {
            showHomeChildPagingUseCase.invoke(pageNumber,site_Slug,cat_Slug).flowOn(dispatcherProvider.io).cachedIn(viewModelScope)
                .catch { e -> _uiState.value = UiState.Error(e.toString()) }
                .collect { _uiState.value = UiState.Success(it) }
